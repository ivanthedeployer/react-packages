...
Tinytest.add('react-meteor-data - basic track', function (test) {
  ...
  ReactDOM.render(React.createElement(Foo, null), div);
...
Tinytest.add('react-meteor-data - track based on props and state', function (test) {
  ...
  var xs = [new ReactiveVar('aaa'), new ReactiveVar('bbb'), new ReactiveVar('ccc')];
  ...
  var comp = ReactDOM.render(React.createElement(Foo, { n: 0 }), div);
  ...
  {
    var comp2 = ReactDOM.render(React.createElement(Foo, { n: 1 }), div);
  ...
  ReactDOM.render(React.createElement(Foo, { n: 0 }), div);
  comp.setState({ m: 0 });
  test.equal(getInnerHtml(div), '<span>AAA</span>');
...
Tinytest.add('react-meteor-data - resubscribe', [function (test, expect) {
  var self = this;
  ...
  self.Foo = React.createClass({
    ...
    mixins: [ReactMeteorData],
    getMeteorData: function getMeteorData() {
      this.handle = Meteor.subscribe("react-meteor-data-mixin-sub", self.num.get());
    ...
    render: function render() {
      return React.createElement(
        'div',
        null,
        _.map(this.data.docs, function (doc) {
          return React.createElement(
            'span',
            { key: doc._id },
            doc._id
  ...
  self.component = ReactDOM.render(React.createElement(self.Foo, null), self.div);
  ...
  test.equal(getInnerHtml(self.div).replace('<span>id1</span>', ''), '<div><span>id2</span></div>');
  ...
  test.equal(self.oldHandle3.subscriptionId, self.component.handle.subscriptionId);
  test.isTrue(self.component.handle.ready());
}, function (test, expect) {
...
Tinytest.add("react-meteor-data - can't call setState in getMeteorData", function (test) {
  var Foo = React.createClass({
    displayName: 'Foo',
  ...
  test.throws(function () {
    ReactDOM.render(React.createElement(Foo, null), document.createElement('div'));
  }, /Can't call `setState`/);
});

Tinytest.add("react-meteor-data - print warning if return cursor from getMeteorData", function (test) {
  var coll = new Mongo.Collection(null);
  var ComponentWithCursor = React.createClass({
    displayName: 'ComponentWithCursor',
  ...
  try {
    ...
    var div = document.createElement("DIV");
    ReactDOM.render(React.createElement(ComponentWithCursor, null), div);
